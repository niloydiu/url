...
export const createUrl = async (req, res) => {
  try {
    const { customUrl, originalUrl } = req.body;
    const trimmedCustomUrl = customUrl ? customUrl.trim() : "";
    let finalCustomUrl = "";

    if (trimmedCustomUrl) {
      if (
        trimmedCustomUrl.startsWith("http://") ||
        trimmedCustomUrl.startsWith("https://")
      ) {
        finalCustomUrl = trimmedCustomUrl;
      } else {
        finalCustomUrl = `https://url-three-sand.vercel.app/${trimmedCustomUrl}`;
      }
    }
